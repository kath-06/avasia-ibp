<template>
	...
	<card>
		...
		<hr v-if="totalAmount != '0.00' || totalAmount != 0">
				...
				<template slot-scope="{ row }">
					...
					<td>
						<span v-for="desc in descriptions"
							...
							<span v-if="desc.reimbursementDescId == row.desc">{{desc.description}}</span>
						</span>
					</td>
					<td align="right">
		...
		<div class="card-header bg-transparent pb-1">
			<div class="text-left">
				<h2 class="display-5"
					id="sideBarTxt">Transportation</h2>
			</div>       
		</div>
		<div class="table-full-width pt-2">
			<paper-table type="hover"
			...
			<div class="form-group row mb-0">
				<div class="col-sm-6 pl-5 pr-5">
					<base-input rows="4"
						...
						id="inputStyle"
						placeholder="Purpose"></base-input>
					<i class="text-red"
...
<script>
	...
	export default {
		...
		methods: {
			...
			// Add new reimbursement descriptions
			addReimDesc(){
				let self = this;
			...
			// Add new reimbursement transportation
			addTransReim(){
				let self = this;
			...
			// Display reimbursement items on table
			getReimItems(page){
				let self = this;
			...
			// Display reimbursement transportation on table
			getTransReim(page){
				let self = this;
				let firstRow = (page - 1) * 10;
				let lastRow = (page * 10);
