<template>
  <div> 
    <card class="pt-4"
      id="textStyle">
      <!-- Cash disbursement type status links -->
      <div class="row">
        <div class="col-12 text-left ml-1 mt-2 mb-2 pt-2">
          <badge class="badge-dot ml-2 pointer">
            ...
            <span class="pointer status"
              id="linkTxt"
              v-on:click="getPaginatedCDTypes(1)">Active</span>
          </badge>
          <badge class="badge-dot ml-3 pointer">
            ...
            <span class="pointer status"
              id="linkTxt"
              v-on:click="getPaginatedCDTypes(0)">Disabled</span>
          </badge>
        </div>
      </div>
      <div class="table-full-width">
        <!-- Cash disbursement type table -->
          ...
          <template slot="table-row"
            ...
            <span v-if="props.column.field == 'action'">
              <span class="pointer ml-2 far fa-edit"
                ...
                v-on:click="openUpdateCDTypeModal(props.row)"
                title="Update"></span>
      ...
      <div class="row pl-4"
        ...
        <span class="col-sm-2">
          <v-select id="inputStyle"
            ...
            v-model="limit"
            :clearable="false"
            @input="getPaginatedCDTypes(1)"></v-select>
        </span>
      </div>
      <hr class="mb-2 mt-2">
      <!-- New cash disbursement type button -->
      <div class="text-right pt-2">
        <base-button v-b-modal.new-cash-disbursement-type-modal
          id="darkBtn"
          class="col-md-4 ml-2 mr-2">New Cash Disbursement Type</base-button>
      </div>
    </card>
    ...
    <!-- UPDATE CASH DISBURSEMENT TYPE MODAL -->
    <b-modal size="md"
      id="update-cash-disbursement-type-modal"
      title="Edit Cash Disbursement Type" centered
      :hide-header-close="true"
      :no-close-on-backdrop="true"
      :no-close-on-esc="true">
      <div class="form-group row mb-0">
      ...
      <template v-slot:modal-footer>
        <div class="w-100">
          <div class="text-right">
            ...
            <base-button id="darkInlineBtn"
              @click.native="$bvModal.hide('update-cash-disbursement-type-modal')"
              :disabled="requestProcessing">Close</base-button>
          </div>
        </div>
      </template>
...
<script>
  ...
  export default {
    ...
    validations: {
      // New cash disbursement type validations
      newType: {
        required,
        alphaNumCustomValidator
      },
      // Edit cash disbursement type validations
      edit: {
        type: {
    ...
    methods: {
      // Display cash disbursement type data on table
      getPaginatedCDTypes(stat) {
        let self = this, cdData = null, cds = [], ids = [];
      ...
      // Add new cash disbursement type data
      addCDType(){
        let self = this;
              ...
              getUserId.get().then((querySnapshot) => {
                ...
                try{
                  ...
                  .set({
                    ...
                    self.$refs['cd-type-table'].reset();
                    self.getPaginatedCDTypes(1);
                    self.$swal({
                      title: 'Success',
      ...
      // Show edit cash disbursement type form
      openUpdateCDTypeModal(row){
        this.edit = {
          id: row.id,
          cdTypeId: row.cashDisbursementTypeId,
          type: row.type
        }
        this.$bvModal.show('update-cash-disbursement-type-modal');
      },
      // Change cash disbursement type details
      updateCDType(){
        let self = this;
        ...
        if(!self.$v.edit.$error){
          ...
          cdData = dbMains.collection('cashDisbursementTypes')
            ...
            .where("cashDisbursementTypeId", "!=", self.edit.cdTypeId);
          cdData.get().then((querySnapshot) => {
            if(!querySnapshot.empty){
              querySnapshot.forEach((doc) => {
                if(doc.exists){
              ...
              try{
                dbMains.collection("cashDisbursementTypes")
                .doc(self.edit.id)
                .update({
                  ...
                  self.edit= {};
                  self.$v.edit.$reset();
                  self.$bvModal.hide('update-cash-disbursement-type-modal');
                  self.requestProcessing = false;
                  self.$refs['cd-type-table'].reset();
                  self.getPaginatedCDTypes(1);
                  self.$swal({
                    title: 'Success',
                    text: 'Cash disbursement type updated successfully!',
                    icon: 'success',
                    confirmButtonColor: '#b80000',
                    confirmButtonText: 'OK'
                  });
      ...
      // Change cash disbursement type status
          ...
          if(result.value){
            try{
              ...
              .update({
                ...
                self.$swal({
                  title: "Success",
                  text: "Cast disbursement type status to " + msg + ".",
                  icon: "success",
                  confirmButtonColor: '#b80000',
                  confirmButtonText: 'OK'
                }).then(() => {
                  self.$refs['cd-type-table'].reset();
                  self.getPaginatedCDTypes(stat);
                });
              });
            }catch(error){
              console.log(error);
