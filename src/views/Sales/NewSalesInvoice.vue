	...
	<card>
		...
		<div class="form-group row mb-0">
			<div class="col-sm-6 pl-5 pr-5">
				...
				<i class="text-red"
					v-if="$v.newSINumber.$anyDirty && !$v.newSINumber.numCustomValidator">Invalid Characters</i>
			</div>
			<div class="col-sm-6 pl-5 pr-5">
		...
		<div class="form-group row mb-0">
			...
			<div class="col-sm-6 pl-5 pr-5">
				...
				<i class="text-red"
					v-if="$v.newDRNumber.$anyDirty && !$v.newDRNumber.alphaNumCustomValidator">Field Required</i>
			</div>
		</div>
		<div class="form-group row mb-0">
			<label class="col-sm-6 col-form-label">
		...
		<div class="form-group row mb-0">
			...
			<div class="col-sm-4 pl-5">
				<v-select id="inputStyle"
					...
					label="name"
					v-model="newInvoiceTo"
					:clearable="false"></v-select>
				<i class="text-red"
					v-if="$v.newInvoiceTo.$anyDirty && !$v.newInvoiceTo.required">Field Required</i>
			</div>
			<div class="col-sm-2 pr-5">
		...
		<div class="text-right">
			<base-button v-b-modal.new-si-item-modal
				class="col-md-2 ml-2"
				id="darkBtn"
			...
			<div class="form-group row mb-0">
				<div class="col-sm-4 pl-5">
					<v-select id="inputStyle"
						...
						v-model="newSIItem"
						:clearable="false"
						@input="changeItem(newSIItem)"></v-select>
					<i class="text-red"
	...
	export default {
		...
		methods: {
			...
			addSIItems(){
				...
				if(!self.$v.newSIItem.$error && !self.$v.newSIQuantity.$error && !self.$v.newSIUnitPrice.$error && !self.$v.newSIAmount.$error && !self.$v.newSIVAT.$error){
					self.requestProcessing = true;
					if(self.siItems.length == 0){
						self.siItems = [];
					}
					let newItem = {
						"rowNum": '',
						...
						"unitPrice": format(self.newSIUnitPrice),
						"amount": format(self.newSIAmount),
						"vat": self.newSIVAT
					}
			...
			getSIItems(){
				...
				if(self.siItems.length != 0){
					...
					for(let a  = 0; a < self.siItems.length; a++){
						self.siItems[a].rowNum = a + 1;
						self.newTotalAmount = parseFloat(self.newTotalAmount) + parseFloat(self.siItems[a].amount.replaceAll(",",""));
					}
			...
			removeItem(item){
				...
				this.$swal({
					...
					if(result.value){
						self.requestProcessing = true;
						self.loading = true;
						self.siItems.splice((parseInt(item.rowNum - 1)), 1);
						self.$swal({
							title: 'Success',
			...
			addSalesInvoice(){
				...
				self.$v.newInvoiceTo.$touch();
				self.$v.newClientPO.$touch();
				self.$v.newTotalAmount.$touch();
				if(!self.$v.newSINumber.$error && !self.$v.newInvoiceDate.$error && !self.$v.newPONumber.$error && !self.$v.newDRNumber.$error && !self.$v.newInvoiceTo.$error && !self.$v.newTotalAmount.$error){
					self.requestProcessing = true;
					let getUserId = dbUsers.collection("users")
						.where("username","==",localStorage.getItem("aisname"));
					...
					getUserId.get().then((querySnapshot) => {
						...
						let dataId = 'SII-' + today.getFullYear() + "" + addZero((today.getMonth() + 1), 2) + "" + today.getDate() + '-' + Math.floor(Math.random() * 1000000);
						...
						dbSales.collection("salesInvoices").doc()
						.set({
							salesInvoiceId: dataId,
							siNumber: self.newSINumber,
							invoiceDate: self.newInvoiceDate,
							...
							customerReference: self.newInvoiceTo,
							status: 1,
							userReference: users[0].userId,
							dateCreated: today,
							dateModified: today
							...
							if(self.siItems.length != 0){
								for(let a = 0; a < self.siItems.length; a++){
									...
									.set({
										...
										description: self.siItems[a].description,
										quantity: self.siItems[a].quantity,
										unitPrice: self.siItems[a].unitPrice,
										amount: self.siItems[a].amount,
										vat: self.siItems[a].vat,
										itemReference: self.siItems[a].itemReference,
										siReference: dataId,
										status: 1,
										userReference: users[0].userId,
										dateCreated: today,
							...
							self.$v.newInvoiceTo.$reset();
							self.$v.newClientPO.$reset();
							self.$v.newTotalAmount.$reset();
							self.requestProcessing = false;
							self.$swal({
								title: 'Success',
								text: 'New sales invoice added successfully!',
								icon: 'success',
								confirmButtonColor: '#b80000',
								confirmButtonText: 'OK'
							}).then(() => {
								self.$router.push("/view/sales/invoice/" + dataId);
							});
						});
					});
				}
