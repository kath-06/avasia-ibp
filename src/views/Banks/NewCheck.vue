		...
		<div class="col-12 container-fluid mt--7">
			<card class="pl-4">
				...
				<div class="form-group row">
					...
					<div class="col-sm-8" v-else>
						<v-select class="form-control-plaintext"
							...
							@input="getCheckSeries(newBank, newBankAccount)"></v-select>
						<i class="text-danger"
					...
					<div class="col-sm-8" v-else>
						<v-select class="form-control-plaintext"
							...
							@input="getCheckNumber(newBank, newBankAccount, newCheckSeries, 'current')"></v-select>
						<i class="text-danger"
					...
					<div class="col-sm-6"
						...
						<span class="text-danger"> No Check Number </span>
					</div>
					...
					<div class="col-sm-3 pt-2">
						...
						<base-button id="goldBtn"
							@click.native="statusCheckNumber(newBank, newBankAccount, 'cancel')"
							v-if="newCheckNumber != ''">Cancelled</base-button>
					</div>
					...
					<div class="col-sm-9">
						<a class="pr-2"
							id="redTxt"
							v-if="checkSeries != '' || activateBankAccounts != '' || checkNumbers != ''"
							@click="getCheckNumber(newBank, newBankAccount, newCheckSeries, 'current')">Current</a>
						<a class="pr-2"
							id="goldTxt"
							v-if="checkSeries != '' || activateBankAccounts != '' || checkNumbers != ''"
							@click="getCheckNumber(newBank, newBankAccount, newCheckSeries, 'skip')">Skip</a>
						<a class="pr-2"
							id="sideBarTxt"
							v-if="checkSeries != '' || activateBankAccounts != '' || checkNumbers != ''"
							@click="getCheckNumber(newBank, newBankAccount, newCheckSeries, 'unused')">Unused</a>
					</div>
					...
					<div class="col-sm-1 pt-1">
						<base-button id="redBtn"
							icon="ni ni-fat-add"
							title="Add Payee" v-b-modal.new-payee-modal></base-button>
					</div>
					<label for="amount"
						class="col-sm-3 col-form-label">
						<b>Description:</b>
					...
					<div class="col-sm-8">
						<textarea row="5"
							v-model="newDesc"
							class="form-control form-control-alternative pl-2"
					...
					<div class="col-sm-8">
						...
						<i class="text-danger"
							v-if="$v.newCheckDate.$anyDirty && !$v.newCheckDate.required">Field Required</i>
					</div>
					<div class="col-sm-12 ml-7 pl-9">
						<input class="mr-2"
							type="radio"
							value="7"
							v-on:click="editDate(7)"
							v-model="dateDays">
						<label class="mr-2">7 Days</label>
						<input class="mr-2"
							type="radio"
							value="15"
							v-on:click="editDate(15)"
							v-model="dateDays">
						<label class="mr-2" >15 Days</label>
						<input class="mr-2"
							type="radio"
							value="30"
							v-on:click="editDate(30)"
							v-model="dateDays">
						<label class="mr-2">30 Days</label>
						<input class="mr-2"
							type="radio"
							value="45"
							v-on:click="editDate(45)"
							v-model="dateDays">
						<label class="mr-2">45 Days</label>
						<input class="mr-2"
							type="radio"
							value=""
							v-on:click="editDate('')"
				...
				<div class="card-header bg-transparent pb-2">
					<div class="text-left">
						<h5 id="sideBarTxt">Particulars</h5>
					</div>       
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-sm-6 pl-5 pr-5 pt-2">
						<base-button block id="redBtn"
			...
			<div class="text-center">
				...
				<base-button id="goldBtn"
					class="col-md-2 ml-2"
					@click.native="cancelNewCheck">Close</base-button>
			</div>
		</card>
			<!-- ADD PAYEE MODAL -->
			<b-modal size="lg"
...
<script>
	...
	const tableColumns = ["#", "Particular", "Reference No.", "Unit Price", "Quantity", "Amount",  "Actions"];
	...
	export default {
		...
		methods: {
			...
			getCheckSeries(bank, bankAccount){
				let self = this;
				let bankCheckSeriesData = db.collection("bankCheckSeries")
					.where("bankReference", "==", bank)
				...
				bankCheckSeriesData.get().then((querySnapshot) => {
					...
					for(let a = 0; a < self.checkSeries.length; a++){
						let num = self.checkSeries[a].bankCheckSeries.search("-");

						self.checkSeries[a].bankCheckSeries = self.checkSeries[a].bankCheckSeries.slice(0, num);
					}
			...
			getParticulars() {
				...
				}else if(self.checkParticulars.length == 0){
					...
					if(result.value){
						...
						checkBankCheckData.get().then((querySnapshot) => {
							if(!querySnapshot.empty){
								querySnapshot.forEach((doc) => {
									if(doc.exists){
										self.$swal({title: 'Invalid', text: 'Check already exists.', icon: 'error', confirmButtonColor: '#b80000', confirmButtonText: 'OK'});
										self.requestProcessing = false;
									}
								});
							}else if(querySnapshot){
								let series = db.collection('bankCheckSeries').where("bankCheckSeriesUid", "==", self.newCheckSeries);
											...
											departmentData.get().then((querySnapshot) =>{
												...
												try{
													...
													.set({
														...
														for(var a = 0; a < self.checkParticulars.length; a++){
															...
															.set({
																...
																idData.get().then((querySnapshot) => {
																	...
																	db.collection("bankCheckSeriesReports").doc(getId)
																	.update({
																	...
																	self.$swal({
																		...
																		self.$router.push("/check/voucher/" + dataId);
																	});
																});
															});
														}
